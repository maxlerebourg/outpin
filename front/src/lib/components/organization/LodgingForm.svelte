...
let { lodging, onFinish } = $props<{ lodging?: Lodging, onFinish: () => void }>()
...
<form class="card bg-base-300 p-4 mb-4 w-full gap-2 max-w-xl m-auto" onsubmit={handleSubmit}>
  ...
  <div class="flex gap-2 input w-full">
    ...
    <label class="flex gap-2">
      ...
      <input type="number" class="w-16" bind:value={newLodging.cost} />
    </label>
  </div>
  <div class="flex gap-2">
    ...
    <label class="input w-full">
      ...
      <input type="datetime-local" bind:value={newLodging.to_at} />
    </label>
  </div>
